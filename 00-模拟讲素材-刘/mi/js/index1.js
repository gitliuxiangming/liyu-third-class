...
//处理导航
function handleNav(){
	var aNavitem = document.querySelectorAll('.header-nav .header-nav-item');
	var aNavContent= document.querySelector('.header .header-nav-content');
	var oBox = document.querySelector('.header-nav-content .container');
	var oLoader= document.querySelector('.header .header-nav-content .loader');
	var timer;
	var timer1;
	...
	for(var i=0;i<aNavitem.length-2;i++){
		...
		aNavitem[i].onmouseenter = function(){
			...
			oBox.innerHTML = '<div class="loader"></div>';
			animate(aNavContent,{height:229},true,function(){
				oLoader.style.display = 'none'
	...
	function hideNavContent(){
		timer = setTimeout(function(){
			...
			animate(aNavContent,{height:0},true,function(){
				aNavContent.style.borderTop = '';
			})
		},500);
	}
	function loadData(index){
		var data = aNanitemData[index];
		var html = '<ul>';
...
//分类处理
function handleCate(){
	var aCateItem = document.querySelectorAll('.cate-box .cate .cate-item');
	var aCateContent = document.querySelector('.cate-box .cate-content');
	var oBox = document.querySelector('.cate-box');
	for(var i=0;i<aCateItem.length;i++){
		// console.log(aCateItem[i])
		aCateItem[i].index = i;
...
function handleElecProduct(){
	var aElecLi = document.querySelectorAll('.elec .tab .tab-item');
	var oElecContent = document.querySelector('.bd .col2 .elec-product');
	loadData(0);
	for(var i=0;i<aElecLi.length;i++){
		aElecLi[i].index = i;
		aElecLi[i].onmouseenter = function(){
			for(var j=0;j<aElecLi.length;j++){
				aElecLi[j].className = 'tab-item';
			}
	...
	function loadData(index){
		...
		for(var i=0;i<data.length-1;i++){
			...
			html += '		<strong>'+data[i].price_now+'</strong><span>&nbsp;元</span>';
		...
		var lastData = data[data.length-1];
			...
			html +='			<i class="iconfont">'+lastData.bottom.iconfont+'</i>';
			html +='		</a>	';
			html +='	</li>';
		oElecContent.innerHTML = html;
	}
}
/*
function handleElecProduct(){
